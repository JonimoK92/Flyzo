...
@extends('template')
@section('titre', 'Résultats de la recherche ')
@section('header_container', 'container-fluid p-5 text-warning bg-voldisp')
@section('header')
<h1 class="text-warning text-center"> Résultats de la recherche </h1>
@endsection
...
<div class = "bg-bl rounded text-center my-4 py-4 text-warning">
    <h2><strong>Vols disponibles </strong></h2>
</div>

    <div class="container bg-warning rounded pt-3 pb-3 mt-5" style="margin-top: -5rem;">
        ...
        <div class="row justify-content-center bg-bl rounded">
            ...
            <form action="{{ route('reservation') }}" method="POST">
                <input type = "hidden" name ="vol_id" value = "{{ $liste->id }} ">
                    <button type="submit" class="btn btn-primary btn-md mb-2">Réserver le vol</button>    
                    @csrf
            </form>
            <form action="{{ route('confirm_panier') }}" method="POST">
            <input type="number" name="vol_quantite" value="1" class="small-input";>
