...
@section('content')
    ...
    <strong class ="text-warning">Réservez votre vol de</strong>
    <strong class="text-primary">{{ $vol->depart_aeroport->nom }} </strong>
    <strong class ="text-warning">à</strong> 
    <strong class="text-primary">{{ $vol->final_aeroport->nom }} </strong>
    <strong class ="text-warning">, votre départ est prévu le</strong> <strong class="text-light">{{ $vol->jour_depart }} </strong>
    <strong class ="text-warning">à</strong> 
    <strong class="text-light">{{ $vol->heure_depart }} </strong>
    <strong class ="text-warning"> et l'arrivée se fera le</strong>
...
<div class="container mt-5 pt-3 pb-3 bg-bl text-warning rounded">
    ...
    <form action="{{ route('passager_reservation') }}" method="POST">
        <div class="mb-3 mt-3">
        ...
        <button type="submit" class="btn btn-primary btn-md">Réserver ce vol</button>
        <p class="mt-2">
            <strong>Prix total :</strong>
            <span class="text-danger"><strong>{{ $vol->prix }} €</strong></span>
        </p>
        @csrf
    </form>
</div>
