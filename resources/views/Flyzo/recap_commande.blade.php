@extends('template')
@section('titre', 'Récapitulatif de la commande fait avec le panier')
...
@section('content')
    <h1><strong>Merci votre réservation a bien été effectué voici le récapitulatif </strong></h1>
    <table class="table table-success">
    <p><strong> Les coordonnées du 1er passager  </strong></p>
    <thead>
      <tr>
    ...
    <tbody>
      <tr>
        <td><strong>{{$reservation->passager->prenom}}</strong></p></td>
        <td><strong>{{ $reservation->passager->nom}}</strong></td>
        <td><strong>{{ $reservation->passager->email}}</strong></td>
      </tr>
    </tbody>
  </table>
    ...
    <h1 class="text-warning" ><strong> Vols réservés </strong></h1>
        <div class="row justify-content-center"> 
            <div class="col-md-2 small-case p-4 my-3 border rounded-4 me-5 mb-2 bg-warning">
                ...
                <hr class="my-5">
                <p>Vol numéro : <strong>{{$reservation->vol->numero}}</strong></p>
                <p><strong> {{ $reservation->vol->depart_aeroport->nom }} - {{ $reservation->vol->final_aeroport->nom }}</strong></p>
                <p>Départ le : <strong> {{ $reservation->vol->jour_depart }} </strong> à  <strong>{{$reservation->vol->heure_depart}}</strong> </p>
                <p>Arrivée le :<strong> {{ $reservation->vol->jour_arrivee }}</strong> à <strong>{{$reservation->vol->heure_arrivee}}</strong> </p>
                <p>Nombre de billets : <strong class="text-danger">{{ $reservation->quantite }}</strong></p>
                <p>Prix : <strong class="text-danger">{{ $reservation->vol->prix;}} €</strong></p>
            </div>
