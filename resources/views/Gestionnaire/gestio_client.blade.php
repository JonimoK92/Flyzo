@extends('template')
@section('titre', 'Visualisez les passagers')
...
<div class = "bg-bl rounded text-center my-4 py-4 text-warning">
    <h2><strong>Liste de tout les passagers/clients </strong></h2>
</div>
<hr>
    ...
    <div class="container mt-5">
        ...
        <div class="row justify-content-center bg-bl rounded"> 
            @foreach ($listepassager as $liste)
            <div class="col-md-2 small-case p-4 my-3 border rounded-4 me-5 mb-2 bg-warning">
                ...
                <hr class="my-5">
                <p>Prénom : <strong>{{$liste->prenom }}</strong></p>
                <p>Nom : <strong>{{ $liste->nom }}</strong> </p>
                <p>Adresse email : <strong>{{ $liste->email }}</strong> </p>
                <form action="{{ route('reservation_client') }}" method="POST">
                    <input type = "hidden" name ="passager_id" value = "{{ $liste->id }} ">
                    <button type="submit" class="btn btn-primary btn-md">Voir les réservations</button>    
                    @csrf
            </form>
            </div>
            @endforeach
